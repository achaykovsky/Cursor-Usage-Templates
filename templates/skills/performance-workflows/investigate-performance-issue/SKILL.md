...
2. **Locate the bottleneck**
   ...
   - If Sentry MCP is configured, pull relevant traces/errors/regression windows and use those IDs as evidence.
...
3. **Propose a fix**
   ...
   - If batching is the likely primary fix (API/DB/queue/ETL), route to `design-batching-strategy` for concrete batch policy and failure semantics.
...
- Treat Sentry/observability MCP reads as evidence-gathering only; do not perform state-changing MCP actions unless explicitly requested.

...
try {
    ...
    if (-not $roots -or $roots.Count -eq 0) { exit 0 }
        ...
        if ($status -or $diffStat -or $staged) {
            ...
            if ($allFiles.Count -gt 1) {
                ...
                foreach ($f in $allFiles) {
                    $group = "chore"
                    if ($f -match 'test[s_]?/|_test\.|\.test\.|spec\.|/tests/') { $group = "test" }
                ...
                if ($groups.Keys.Count -gt 1) {
                    ...
                    foreach ($key in @("feat", "fix", "docs", "test", "chore")) {
                        if ($groups[$key]) {
                            $msg = switch ($key) {
                                "feat" { "feat: add or update feature" }
                                "fix" { "fix: fix bug" }
                                "docs" { "docs: update documentation" }
                                "test" { "test: add or update tests" }
                                default { "chore: update config or misc" }
                            }

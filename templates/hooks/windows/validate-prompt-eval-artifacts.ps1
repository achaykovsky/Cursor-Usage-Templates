...
try {
    ...
    $isPromptEval = $filePath -match '(?i)(templates[/\\]ai-runtime[/\\]eval[/\\].*\.json$|[/\\]ai-runtime[/\\]eval[/\\].*\.json$)'
    if (-not $isPromptEval) { exit 0 }
    if ($filePath -match '(?i)(\.schema\.json$|prompt-eval\.schema\.json$|eval-baseline\.schema\.json$|llm-judge-calibration\.schema\.json$)') { exit 0 }
    if ($filePath -match '(?i)([/\\]fixtures[/\\])') { exit 0 }
    ...
    $kind = if ($filePath -match '(?i)([/\\]calibration[/\\])') { "judge-calibration" } elseif ($filePath -match '(?i)([/\\]baselines[/\\]|baseline\.json$)') { "eval-baseline" } else { "prompt-eval" }
    $stderr = & $py $validator $kind $fullPath 2>&1
    if ($LASTEXITCODE -ne 0) {
        $msg = ($stderr | Out-String).Trim()
        if ([string]::IsNullOrWhiteSpace($msg)) { $msg = "prompt eval $kind validation failed" }
        Write-HookError "validate-prompt-eval-artifacts: $msg"
    }
} catch {

...
if [[ ! "$file_path" =~ templates[/\\]ai-runtime[/\\]policy[/\\].*\.json$ ]]; then exit 0; fi
if [[ "$file_path" =~ tool-risk-catalog\.json$ ]]; then exit 0; fi
...
if ! stderr=$("$py" "$validator" policy "$full_path" 2>&1); then
  msg=$(echo "$stderr" | tr -d '\r' | sed '/^$/d' | head -n 1)
  [[ -z "$msg" ]] && msg="policy validation failed"
  echo "validate-ai-policy-schema: $msg" >&2
fi
